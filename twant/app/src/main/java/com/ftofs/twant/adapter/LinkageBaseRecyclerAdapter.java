 ...
 * @date 2018/5/22
 */
public abstract class LinkageBaseRecyclerAdapter<T> extends RecyclerView.Adapter<LinkageBaseViewHolder<T>> {

    protected List<T> mListData;

    private LinkageBaseRecyclerAdapter.OnItemClickListener<T> onItemClickListener;
    private LinkageBaseRecyclerAdapter.OnItemLongClickListener<T> mOnItemLongClickListener;
    ...
    @Override
    public void onBindViewHolder(LinkageBaseViewHolder<T> holder, int position) {
        holder.refresh(mListData.get(position), position);
    }

    @Override
    public void onBindViewHolder(LinkageBaseViewHolder<T> holder, int position, List<Object> payloads) {
        if (payloads.isEmpty()){
            holder.refresh(mListData.get(position), position);
        }else{
