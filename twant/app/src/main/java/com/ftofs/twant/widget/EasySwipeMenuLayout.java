...
public class EasySwipeMenuLayout extends ViewGroup {
    ...
    private static EasySwipeMenuState mStateCache;
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if (mViewCache != null) {
                    if (mViewCache != this) {
                        mViewCache.handlerSwipeMenu(EasySwipeMenuState.CLOSE);
                    }
                    // Log.i(TAG, ">>>有菜单被打开");
...
//                }
    ...
    public void handlerSwipeMenu(EasySwipeMenuState result) {
        if (result == EasySwipeMenuState.LEFTOPEN) {
            mScroller.startScroll(getScrollX(), 0, mLeftView.getLeft() - getScrollX(), 0);
            mViewCache = this;
            mStateCache = result;
        } else if (result == EasySwipeMenuState.RIGHTOPEN) {
            mViewCache = this;
     ...
     * @return
     */
    private EasySwipeMenuState isShouldOpen(int scrollX) {
        ...
        if (finalyDistanceX < 0) {
            ...
            if (getScrollX() < 0 && mLeftView != null) {
                if (Math.abs(mLeftView.getWidth() * mFraction) < Math.abs(getScrollX())) {
                    return EasySwipeMenuState.LEFTOPEN;
                }
            }
            //2、关闭右边按钮

            if (getScrollX() > 0 && mRightView != null) {
                return EasySwipeMenuState.CLOSE;
            }
            ...
            if (getScrollX() > 0 && mRightView != null) {
                ...
                if (Math.abs(mRightView.getWidth() * mFraction) < Math.abs(getScrollX())) {
                    return EasySwipeMenuState.RIGHTOPEN;
                }
    ...
    public void resetStatus() {
        if (mViewCache != null) {
            if (mStateCache != null && mStateCache != EasySwipeMenuState.CLOSE && mScroller != null) {
    ...
    public static EasySwipeMenuState getStateCache() {
        return mStateCache;
    }
